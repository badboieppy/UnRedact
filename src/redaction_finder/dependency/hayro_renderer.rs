...
use crate::redaction_finder::service::redaction_finder_entry::RedactionError;
use crate::redaction_finder::types::{PdfRenderer, RenderedPage};
...
impl HayroRenderer {
    ...
    pub fn new_from_bytes(bytes: &[u8]) -> Result<Self, RedactionError> {
        ...
        let pdf = Pdf::new(data)
            .map_err(|e| RedactionError::Render(format!("hayro_pdf_load_failed:{e:?}")))?;
