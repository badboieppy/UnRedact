...
$bundles = @(
  [ordered]@{
    ...
    intent="neutral"
    purpose="Define the canonical anchor decision schema with deterministic source labels, one terminal decision reason code, and per-side plus row confidence values."
    value="Makes anchor decisions machine-parseable and trace-complete without inferred behavior from free-form diagnostics."
    scope_in=@(
  ...
  [ordered]@{
    ...
    intent="improve"
    purpose="Lock deterministic source-resolution semantics for run_exact and run_prefix_projection with explicit provenance and newline and punctuation preserving exactness."
    ...
    purpose_demo="Running the same input twice produces identical per-side source labels and coordinate provenance, and exact matches keep punctuation and newline sensitive text semantics."
  ...
  [ordered]@{
    ...
    value="Prevents output contract divergence across ingress and egress surfaces and keeps artifact publication deterministic."
    scope_in=@(
  ...
  [ordered]@{
    ...
    intent="neutral"
    purpose="Make benchmark hard-pass logic machine-checkable by declared bundle intent with 1 percent margin bands."
...
foreach ($bundle in $bundles) {
  ...
  $contractsConsumedBullets = ToBullets $bundle.contracts_consumed
  $contractsIntroducedBullets = ToBullets $bundle.contracts_introduced
  ...
  $requiredCommandsBullets = ToBullets $requiredCommands
